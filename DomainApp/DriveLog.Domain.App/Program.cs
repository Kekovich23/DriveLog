...
var race = new Race(Guid.CreateVersion7(), track, new(DateTimeOffset.UtcNow));
...
for (var i = 0; i < 3; i++) {
    race.RecordLapTime(driver, new(TimeSpan.FromMilliseconds(90000 + rnd.Next(30000))));
    var raceLap = race.Entries.First(e => e.Driver.Id == driver.Id).Laps.Last();
